...
const pool = mysql.createPool({
    ...
    port: '3306',
    user: 'freedb_defaultdb',
    password: 'tg$QP6u4MHxNcrb',
    database: 'freedb_databases'
});
        ...
        switch (tipo) {
            case 'cadastro':
                var [rows, fields] = await pool.query(
                    "insert into `freedb_databases`.`tbl_cadastro` (`nome`, `login`, `senha`) values (?, ?, ?);",
                    [nome, login, senha]
                );
                ...
                case 'login':
                    try {
                        var [rows, fields] = await pool.query(
                            "select * from `freedb_databases`.`tbl_cadastro` where `nome` = ? and `login` = ? and `senha` = ?;",
                            [nome, login, senha]
                        );
                        if (rows.length >= 1) {
                ...
                var strSql = "select * from `freedb_databases`.`tbl_cadastro` where" + 
                ...
                break;
                case 'atualizacao':
                    var strSql = "select * from `freedb_databases`.`tbl_cadastro`;";
                    var [rows, fields] = await pool.query(strSql);
                    if (rows.length > 0) {
                        res.json({ 
                ...
                case 'atualizar':
                    ...
                    var strSql = "update `freedb_databases`.`tbl_cadastro` set " + 
